...
`postgres.yml`. We can switch between them by changing the selected `db` option.

Default `db/mysql.yml`:

```{r, echo=FALSE, results='asis'}
fn <- system.file("examples", "db", "mysql.yml", package = "hydraR")
cat("```yaml\n")
cat(paste(readLines(fn), collapse = "\n"))
cat("\n```\n")
```
...
```{r, echo=FALSE, results='asis'}
fn <- system.file("examples", "db", "postgres.yml", package = "hydraR")
cat("```yaml\n")
cat(paste(readLines(fn), collapse = "\n"))
cat("\n```\n")
```
