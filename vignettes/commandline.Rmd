...
vignette: >
  %\VignetteIndexEntry{Command line}
...
Sometimes, the analyst wants to run an `R` script from the shell and override a few
configuration values at runtime, without editing YAML files. They may also want a
multi-run call that executes the same script over a grid of configuration
parameters.

This workflow can be achieved using `Rscript` and the `hydraR::main()` function. 

## Minimal example

When you only need one run with default parameters, a minimal config and a single
`Rscript` call keep things simple and reproducible.
...
Using the same `minimal` config as before, we can do a single parameter override:
...
If many functions share the same config, setting `hydraR.main.*` options lets you avoid repeating `config_path` and `config_name` on every call.
